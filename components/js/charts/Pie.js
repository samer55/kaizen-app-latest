...
class Pie extends React.Component {
  ...
  edit(item) { this.setState({edit:item.number,selecteds:true});  this.props.edits(item.number,item.uid,item.name); }
        ...
        <View style={{position: 'absolute', top:margin, left: 2*margin + this.props.pieWidth}}>
          {
            this.props.data.map( (item, index) =>
            {
              ...
              return (
                <TouchableWithoutFeedback key={index} onPress={() =>{ this._onPieItemSelected(index),this.edit(item)}}>
                  <View>
