...
const title =this.state.title
     ...
     firebaseApp.database().ref('goals/'+ `/${name}/` + `/${uid}/` ).orderByChild('Category').equalTo(this.state.title).on('value',
     (snapshot) => {
     ...
     firebaseApp.database().ref('goals/'+ `/${name}/` + `/${uid}/`+'/date/' ).on('value',
     (snapshot) => {
       if (snapshot.val()) {
...
if (this.state.puid.length > 0) {
  firebaseApp.database().ref('goals/'+ `/${name}/` + `/${uid}/` +   `/${this.state.puid}`).remove()
...
updatedAt: firebase.database.ServerValue.TIMESTAMP,
          ...
          updates['goals/'+ `/${name}/` + `/${uid}/`  + '/date/'] = postData
...
puid:this.state.puid,
          ...
          let updates = {}
          updates['goals/'+ `/${name}/` + `/${uid}/`  + this.state.puid] = postData
...
value={this.state.edit}
      ...
      />
   <Button primary bordered style={{justifyContent:'center',alignItems: 'center',padding: 25}} onPress={this.showDateTimePicker} >
   {this.state.date.length > 0 ?<Text style={{color:'#4286f4'}}> {this.state.date}</Text>:<Text style={{color:'#4286f4'}}>
Add Date
   </Text>}
   </Button>
   <DateTimePicker
