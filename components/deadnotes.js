...
class Deadnotes extends Component {
  state = {
...
const title =this.state.title
     ...
     firebaseApp.database().ref('Deadline/'+ `/${name}/` + `/${uid}/` ).orderByChild('cat').equalTo(this.state.title).on('value',
     (snapshot) => {
   ...
   if (this.state.puid.length > 0) {
             firebaseApp.database().ref('Deadline/'+ `/${name}/` + `/${uid}/` +   `/${this.state.puid}`).remove()
   }
...
puid:this.state.puid,
          ...
          updates['Deadline/'+ `/${name}/` + `/${uid}/` + this.state.puid] = postData
...
value={this.state.goals}
         ...
         >
         <Button primary bordered style={{justifyContent:'center',alignItems: 'center',padding: 25}} onPress={this.showDateTimePicker} >
         {this.state.editdate.length > 0 ?<Text style={{color:'#4286f4'}}> {this.state.editdate}</Text>:<Text style={{color:'#4286f4'}}>
      Add Date
         </Text>}
         </Button>
         <DateTimePicker
              isVisible={isDateTimePickerVisible}
              onConfirm={this.handleDateedit}
              onCancel={this.hideDateTimePicker}
            />
           <Input
             style={{
...
<Right>
<Text style={{fontSize:12,fontWeight:'500',color:'red'}}>{data.date}</Text>
</Right>
</CardItem>
